...
def _extract_error_code(query_text):
    normalized = query_text.lower()
    match = re.search(r"error\s*(?:code\s*)?(\d{3})", normalized)
    return match.group(1) if match else None


def retrieve_chunks(query_text, top_k=3, similarity_threshold=0.85):
    query_embedding = embed_query(query_text)
    error_code = _extract_error_code(query_text)

    scored = []
    for chunk in RUNBOOK_CHUNKS:
        score = cosine_similarity(query_embedding, chunk.embedding)
        if error_code and error_code in chunk.content.lower():
            # Boost exact error-code matches so highly specific tickets do not get lost
            score += 0.15

        if score >= similarity_threshold or (error_code and error_code in chunk.content.lower()):
